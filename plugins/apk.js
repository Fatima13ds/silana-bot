...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
  ...
  let res = await apk(text);
        ...
        if (res.size > 993000000) {
            m.react(error)
            throw '*ملف APK كبير جدًا. الحد الأقصى لحجم التنزيل هو 150 ميجابايت*.';
          }
  ...
  await conn.sendMessage(m.chat, {
    text: `*جاري تحميل* ${info.name}...\n\n*متصباميش كثر على البوت تسنا واحد شوية وغادي يحط ليك تطبيق لي بغيتي😄❤️*`,
  });
...
async function apkinfo(url) {
  ...
  if (!download) throw '*تعذر تحميل التطبيق انا آسف*';
